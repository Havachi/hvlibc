...
static int _print_number(FILE *stream, unsigned long long value, int base, bool is_upper) {
	char buffer[64];
	int i = 0;
	int printed = 0;
	...
	for (const char *p = format; *p != '\0'; p++) {
		...
		switch (*p) {
			...
			case 'i': {
				...
				int printed = _print_number(s, (unsigned long long) n, 10, false);
			...
			case 'u': {
				...
				int printed = _print_number(s, n, 10, false);
				if (printed < 0) return -1;
				total += printed;
				break;
			}
			case 'x': {
				unsigned int n = va_arg(arg, unsigned int);
				int printed = _print_number(s, n, 16, false);
				if (printed < 0) return -1;
				total += printed;
				break;
			}
			case 'X': {
				unsigned int n = va_arg(arg, unsigned int);
				int printed = _print_number(s, n, 16, true);
				if (printed < 0) return -1;
				total += printed;
				break;
			}
			case 'p': {
				...
				total += 2;
				int printed = _print_number(s, (unsigned long long)ptr, 16, false);
				if (printed < 0) return -1;
				total += printed;
				break;
			}
